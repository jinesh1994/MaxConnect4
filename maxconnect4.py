...
def interactiveGame(currentGame, firstMovePlayer):
    # Fill me in
    currentGame.currentTurn = 1
    if firstMovePlayer == 'human-first':
        currentGame.currentTurn = 2
    while not currentGame.checkPieceCount() == 42:
        if currentGame.currentTurn == 1:
            ...
            try:
                ...
                pass
                # sys.exit('Error opening output file.')
            # sys.exit('human player is currently not implemented')
...
def main(argv):
    ...
    if game_mode == 'interactive':
        ...
        if not firstMovePlayer == 'computer-first' and not firstMovePlayer == 'human-first':
